      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8">
          <div>
             ...
             <h2 className="text-4xl font-bold tracking-widest text-white display-font">ENGINE <span className="text-primary">ROOM</span></h2>
          </div>
          <div className="text-right border-r-2 border-primary pr-6 hidden md:block">
            <span className="text-[10px] text-primary block mb-1 font-display">SYSTEM_RESOURCES</span>
            <span className="text-xs text-white/50 font-mono">V_CORE_LOAD: 12% | THREADS: 64</span>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {TECH_STACK.map((tech) => (
            <HoloCard key={tech.name} className="p-8 flex flex-col items-center group transition-all hover:scale-105">
              ...
              <span className="text-[9px] display-font text-white mb-4">{tech.name}</span>

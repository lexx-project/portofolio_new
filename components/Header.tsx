      ...
      <div className="mx-auto flex h-full max-w-full items-center justify-between px-4 sm:px-8">
        ...
        <div className="flex items-center gap-6 cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
          ...
          <div className="flex flex-col">
            <span className="display-font text-xs font-bold leading-none text-primary">ARCHITECT_OS</span>
            <span className="text-[10px] text-primary/60 font-mono mt-1">V_2.0.26_STABLE</span>
          </div>
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {NAV_ITEMS.map((item) => (
            ...
            >
              <span className="text-[8px] opacity-50 group-hover:opacity-100 transition-opacity">{item.number}</span> {item.label}
            </button>
          ))}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-4">
          <div className="flex flex-col items-end mr-4">
            ...
            <span className="text-[9px] text-primary/50 mt-1 font-mono">LINK_ESTABLISHED</span>
          </div>
        ...
        <div className="absolute top-16 left-0 w-full bg-black/95 border-b border-primary/20 p-4 lg:hidden flex flex-col gap-4">
          {NAV_ITEMS.map((item) => (
            ...
            >
              ...
              <span className="display-font text-white text-sm tracking-widest">{item.label}</span>
            </button>
          ))}
        </div>
      )}
